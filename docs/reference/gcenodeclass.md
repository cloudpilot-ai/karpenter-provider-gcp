...
| `confidentialInstanceConfig` _[ConfidentialInstanceConfig](#confidentialinstanceconfig)_ | ConfidentialInstanceConfig enables Confidential VM for provisioned nodes,<br />providing in-use memory encryption via AMD SEV / SEV-SNP or Intel TDX.<br />Only supported on specific machine families; incompatible machine types<br />are rejected by GCE at instance creation. When enabled, scheduling<br />onHostMaintenance is forced to TERMINATE since Confidential VMs cannot<br />live-migrate. |  | Optional: \{\} <br /> |
