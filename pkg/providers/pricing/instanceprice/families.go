...
var gpuSKUPrefixes = []prefixEntry{
	...
	{"nvidia-h100-plus-80gb", "Nvidia H100 80GB Plus GPU"},
...
// lssdGenericRateFamilies is the set of machine families whose -lssd variants are billed
// using the generic "SSD backed Local Storage" SKU rather than a family-specific one.
// These families are NOT listed in localSSDFamilySKUPrefixes; their -lssd variants use
// the same generic rate as locally-attached SSDs on standard machines.
//
// For families IN localSSDFamilySKUPrefixes (c4/c4a/c4d/h4d): if the family SSD SKU is
// missing in a region, the SSD is treated as bundled in the VM price ($0 additional).
//
// Source: https://github.com/Cyclenerd/google-cloud-pricing-cost-calculator/pull/465
var lssdGenericRateFamilies = map[string]bool{
	"c3":  true,
	"c3d": true,
}
...
//   - a3-megagpu*: name starts with "a3-" → ExtractFamily returns "a3" (regular A3),
//     but these machines use A3Plus CPU/RAM billing SKUs (same as a3-plus*).
...
// gpuBillingFraction maps machine type names to the fraction of the reported
// GuestAcceleratorCount that is actually billed. Non-1 values occur when a
// single physical GPU is partitioned across multiple VM sizes.
//
// G4 instances use RTX Pro 6000 GPU partitioning: g4-standard-6/12/24 each
// share one physical GPU at 1/8, 1/4, and 1/2 allocation respectively. The
// Compute API reports GuestAcceleratorCount=1 for all sub-48 sizes, but GCP
// bills at the fractional rate.
//
// Source: https://cloud.google.com/compute/docs/gpus/g4-vms#machine_type_gpu_table
var gpuBillingFraction = map[string]float64{
	"g4-standard-6":  0.125, // 1/8 GPU
	"g4-standard-12": 0.25,  // 1/4 GPU
	"g4-standard-24": 0.5,   // 1/2 GPU
}
