...
// arm64MachineType is a c4a machine used when creating ARM64 node pool templates.
// c4a (Google Axion) is available in ~27 regions; t2a (Ampere Altra) only covers 3.
// The actual machine type used for provisioned nodes comes from the instance template
// generated by GKE, not this constant.
...
func (p *DefaultProvider) Create(ctx context.Context) error {
	...
	// ARM64 pools are best-effort: not all regions support ARM64 machine types.
	// Locations is left nil so GKE defaults to the cluster's zones; GKE will return
	// an error if the machine type is unavailable, which is handled gracefully below.
	if err := p.ensureKarpenterNodePoolTemplate(ctx, KarpenterDefaultNodePoolTemplateImageType, KarpenterCOSARM64NodePoolTemplate, p.defaultServiceAccount, arm64MachineType); err != nil {
		log.FromContext(ctx).Error(err, "ARM64 COS node pool template unavailable, ARM64 COS provisioning disabled")
	}
	if err := p.ensureKarpenterNodePoolTemplate(ctx, KarpenterUbuntuNodePoolTemplateImageType, KarpenterUbuntuARM64NodePoolTemplate, p.defaultServiceAccount, arm64MachineType); err != nil {
		log.FromContext(ctx).Error(err, "ARM64 Ubuntu node pool template unavailable, ARM64 Ubuntu provisioning disabled")
	}
...
func (p *DefaultProvider) GetBootstrapMetadata(ctx context.Context) (map[string]*compute.InstanceTemplate, error) {
	...
	for _, name := range []string{KarpenterCOSARM64NodePoolTemplate, KarpenterUbuntuARM64NodePoolTemplate} {
		...
		if err != nil {
			var gcpErr *googleapi.Error
			if errors.As(err, &gcpErr) && gcpErr.Code == http.StatusNotFound {
				log.FromContext(ctx).V(1).Info("ARM64 node pool not found, skipping", "nodePoolName", name)
			} else {
				log.FromContext(ctx).Error(err, "failed to fetch ARM64 instance template", "nodePoolName", name)
			}
			continue
		}
		if t != nil {
