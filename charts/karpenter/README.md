...
| spotPreemptionNotice.image.repository | string | `"registry.k8s.io/node-problem-detector/node-problem-detector"` | Repository for the node-problem-detector image running the detection plugin. The plugin reads the metadata server over bash's /dev/tcp, because this image ships no curl, wget or python. An override must provide bash with /dev/tcp support. |
