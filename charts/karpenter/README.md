...
| controller.env | list | `[]` |  |
| controller.featureGates.capacityBuffer | bool | `false` | capacityBuffer is ALPHA and is disabled by default. Setting this to true enables CapacityBuffer support for pre-provisioning spare capacity and installs the CapacityBuffer CRD (skipped on GKE clusters that already provide it natively — see charts/karpenter/templates/autoscaling.x-k8s.io_capacitybuffers.yaml). |
