...
| node-problem-detector.settings.custom_monitor_definitions | object | `{"check-preemption.sh":"#!/usr/bin/env bash\nset -uo pipefail\n\nHOST=\"${METADATA_HOST:-169.254.169.254}\"\nPORT=\"${METADATA_PORT:-80}\"\nREQUEST_PATH=\"/computeMetadata/v1/instance/preempted\"\nTIMEOUT=\"${REQUEST_TIMEOUT_SECONDS:-5}\"\n\nunknown() {\n  echo \"$1\"\n  exit 2\n}\n\nexec 3<>\"/dev/tcp/${HOST}/${PORT}\" 2>/dev/null ||\n  unknown \"cannot reach metadata server at ${HOST}:${PORT}\"\n\nprintf 'GET %s HTTP/1.0\\r\\nHost: metadata.google.internal\\r\\nMetadata-Flavor: Google\\r\\nConnection: close\\r\\n\\r\\n' \\\n  \"${REQUEST_PATH}\" >&3 2>/dev/null || unknown \"metadata request failed\"\n\nresponse=\"\"\nline=\"\"\nread_status=0\nwhile true; do\n  IFS= read -r -t \"${TIMEOUT}\" line <&3\n  read_status=$?\n  if [ \"${read_status}\" -eq 0 ]; then\n    response+=\"${line}\"$'\\n'\n    continue\n  fi\n  [ -n \"${line}\" ] && response+=\"${line}\"$'\\n'\n  break\ndone\nexec 3<&-\n\nif [ \"${read_status}\" -gt 128 ]; then\n  unknown \"timed out reading from metadata server after ${TIMEOUT}s\"\nfi\n[ -n \"${response}\" ] || unknown \"empty response from metadata server\"\n\nresponse=\"${response//$'\\r'/}\"\n\nstatus_line=\"${response%%$'\\n'*}\"\ncase \"${status_line}\" in\nHTTP/1.[01]\\ 200*) ;;\n*) unknown \"unexpected metadata response: ${status_line}\" ;;\nesac\n\ncase \"${response}\" in\n*$'\\n\\n'*) body=\"${response#*$'\\n\\n'}\" ;;\n*) unknown \"malformed metadata response: no header terminator\" ;;\nesac\n\nbody=\"${body#\"${body%%[![:space:]]*}\"}\"\nbody=\"${body%\"${body##*[![:space:]]}\"}\"\n\ncase \"${body}\" in\nTRUE | true)\n  echo \"instance is being preempted\"\n  exit 1\n  ;;\nFALSE | false) exit 0 ;;\n*) unknown \"unexpected instance/preempted value: '${body}'\" ;;\nesac\n","gce-preemption-monitor.json":"{\n  \"plugin\": \"custom\",\n  \"pluginConfig\": {\n    \"invoke_interval\": \"2s\",\n    \"timeout\": \"15s\",\n    \"max_output_length\": 120,\n    \"concurrency\": 1,\n    \"skip_initial_status\": true\n  },\n  \"source\": \"gce-preemption-monitor\",\n  \"metricsReporting\": true,\n  \"conditions\": [\n    {\n      \"type\": \"GCESpotPreempting\",\n      \"reason\": \"NoPreemptionNotice\",\n      \"message\": \"no GCE Spot preemption notice\"\n    }\n  ],\n  \"rules\": [\n    {\n      \"type\": \"permanent\",\n      \"condition\": \"GCESpotPreempting\",\n      \"reason\": \"SpotPreemptionImminent\",\n      \"path\": \"/custom-config/check-preemption.sh\",\n      \"timeout\": \"10s\"\n    }\n  ]\n}\n"}` | Monitor config and probe script, mounted at /custom-config. The probe talks to the metadata server over bash's /dev/tcp because the node-problem-detector image ships no curl, wget or python; see docs/spot-preemption.md. |
| node-problem-detector.settings.custom_plugin_monitors | list | `["/custom-config/gce-preemption-monitor.json"]` | Activates the preemption monitor defined in `custom_monitor_definitions`. |
| node-problem-detector.settings.log_monitors | list | `[]` | Run only the preemption plugin. The GKE-managed node-problem-detector already covers the kernel and runtime log monitors on Standard node pools. |
