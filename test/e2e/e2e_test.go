...
func runProvisioningTest(t *testing.T, tc provisioningCase) {
	...
	t.Cleanup(func() {
		...
		defer cleanupCancel()
		require.NoError(t, deleteDeployment(cleanupCtx, deploymentName))
		require.NoError(t, deleteNodePool(cleanupCtx, nodePoolName))
		require.NoError(t, deleteNodeClass(cleanupCtx, nodeClassName))
	})
	...
	assert.True(t, nodeReady, "expected node to be Ready")
	...
	assert.Contains(t, tc.instanceTypes, node.Labels[corev1.LabelInstanceTypeStable])
...
func createNodeClass(ctx context.Context, name string) error {
	object := &unstructured.Unstructured{Object: map[string]any{
		...
		"spec": map[string]any{
			...
			"subnetRangeName": testEnv.podsRangeName,
		},
	}}
...
func createNodePool(ctx context.Context, name, nodeClassName string, tc provisioningCase) error {
	requirements := []any{
		...
		map[string]any{
			...
			"operator": "In",
			"values":   stringSliceToAny(tc.instanceTypes),
		},
		map[string]any{
...
func createDeployment(ctx context.Context, name, appLabel, nodePoolName string) error {
	...
	_, err := testEnv.clientset.AppsV1().Deployments(testEnv.testNamespace).Create(ctx, deployment, metav1.CreateOptions{})
	return err
}
...
func waitForRunningPod(ctx context.Context, appLabel string) (*corev1.Pod, error) {
	...
	err := wait.PollUntilContextTimeout(ctx, 5*time.Second, provisioningTimeout, true, func(ctx context.Context) (bool, error) {
		...
		pods, err := testEnv.clientset.CoreV1().Pods(testEnv.testNamespace).List(ctx, metav1.ListOptions{LabelSelector: fmt.Sprintf("app=%s", appLabel)})
		if err != nil {
			return false, err
		}
...
func waitForNodeRemoval(ctx context.Context, nodeName string) error {
	return wait.PollUntilContextTimeout(ctx, 10*time.Second, nodeCleanupTimeout, true, func(ctx context.Context) (bool, error) {
...
func deleteDeployment(ctx context.Context, name string) error {
	err := testEnv.clientset.AppsV1().Deployments(testEnv.testNamespace).Delete(ctx, name, metav1.DeleteOptions{})
	if apierrors.IsNotFound(err) {
		return nil
	}
	return err
