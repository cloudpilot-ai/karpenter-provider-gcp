...
- Controllers: nodepooltemplate and CSR approver not registered in `self-hosted` mode; `BootstrapConfigReady` requires `startupScript` in `self-hosted` mode (including the byte-length check with a multi-byte script) and rejects it in `gke` mode; image reconciler errors on non-`id` terms without touching GKE paths.
