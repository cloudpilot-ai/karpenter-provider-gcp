...
This proposal adds a `PROVISION_MODE` operator setting with values `gke` (default, current behavior) and `self-hosted`. In `self-hosted` mode the provider never calls the Container API; node bootstrap is owned by the instance image or by a startup script supplied externally (typically generated by the distribution) through a new `GCENodeClass` field, `spec.startupScript`, written verbatim to the instance's `startup-script` metadata key, which the GCE guest agent executes natively on every supported image (no cloud-init required). Whether the control plane is GKE is a property of the deployment (like `CLUSTER_NAME`), so it is declared once at the deployment, not inferred per NodeClass.

This mirrors the AWS provider's `amiFamily: Custom` passthrough, which self-hosted distributions already build on today. "Custom" refers to bootstrap ownership, not the image: `id:` selector terms accept any GCE image, including the public `cos-cloud` and `ubuntu-os-cloud` catalogs. A reference implementation of the same approach for Azure exists in [Azure/karpenter-provider-azure#1765](https://github.com/Azure/karpenter-provider-azure/pull/1765).
...
- Automated self-hosted e2e in CI for the MVP (initial validation is manual).
...
| Bootstrap material                                    | cloned GKE node pool template, provider-patched     | `startupScript` or baked into the image                     |
...
**Runtime:** the self-hosted launch path validates the rendered metadata payload — all keys and values, in UTF-8 bytes — against the GCE limits above before `Instances.Insert` (OpenAPI `maxLength` counts characters, not bytes). `startupScript` is optional: images with baked-in bootstrap need none. In `gke` mode the field is ignored.
...
- Bootstrap pool discovery and source template metadata. Instance metadata is produced by a dedicated self-hosted metadata builder that renders **only** `startup-script` (when set), `cluster-name`, `spec.metadata` keys, and karpenter-owned keys, never `kube-env`, `kube-labels`, or `kubelet-config` (the current typed model always renders those, so this is a separate construction path, not a nil source).
...
- **nodeclass status controller**: the image reconciler enforces `id`-only resolution in `self-hosted` mode (see Container API Isolation).
...
- The GCE guest agent runs the startup script on **every boot**, not only the first; the script must be idempotent.
- The image must include the Google guest environment (all public-catalog images do); without it the `startup-script` metadata key never executes.
...
|---------------------------------------------------------------|------------|--------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------|
| "Node never joined" reports for startup-script bugs           | High       | Support noise                                                | Documented registration contract + example                                                                               |
...
| A GKE user sets `startupScript` expecting augmentation        | Medium     | Confusion                                                    | Field documented as self-hosted-only; non-goal reserves a separate field for augmentation                                |
...
| Same-project/location clusters with colliding sanitized names | Low        | Cross-cluster GC deletes the other cluster's Karpenter nodes | Exact `cluster-name` metadata match required before List/GC; the sanitized label is only a coarse filter                 |
...
- Instance listing/GC: instances with missing or mismatched `cluster-name` metadata are excluded in both modes (including the `a.b.com` vs `a-b.com` label collision).
- Controllers: nodepooltemplate and CSR approver not registered in `self-hosted` mode; image reconciler errors on non-`id` terms without touching GKE paths.

### E2E / Integration Tests

- Manual validation against at least one self-hosted distribution (kOps, per #297): provision, register, consolidate, Spot interruption (with kubelet graceful shutdown enabled by the startup script).
...
- [ ] The self-hosted launch path validates the rendered metadata payload against the GCE byte limits.
...
- [ ] The GC label carries the sanitized cluster name; `cluster-name` metadata carries the exact name; `syncInstances` admits only exact metadata matches.
...
- [ ] Docs: self-hosted guide with the registration contract, the IAM requirements list, at least one distribution example, and chart guidance for non-GKE clusters (`controller.priorityClassName` defaults to GKE-only `gmp-critical`; metadata-server ADC via `credentials.enabled=false`).
...
Everything specified in this proposal: the operator option and validation, `spec.startupScript` with its CEL rules, the self-hosted metadata builder, controller gating (nodepooltemplate, CSR approver), `id`-only image terms, and manual kOps validation.
...
### Phase 4 — Automated self-hosted e2e

Automate the manual kOps validation from the Test Plan as a CI-runnable suite, guarding the mode against regressions.
...
### Separate NodeClass CRD for self-hosted clusters

Raised in review: a second kind (e.g. `SelfHostedNodeClass`) could separate the two code paths entirely. Rejected for now: the resource model is the same GCE VM in both modes (disks, networking, shielded/confidential config, metadata, labels, images, binpacking hints, drift, status, termination), and the differences concentrate in bootstrap ownership and validation, which the feature matrix makes explicit. A separate kind would add a second public API plus NodeClass-kind dispatch throughout the provider, for no capability the deployment-wide mode does not already provide. AWS (`amiFamily: Custom`) and IBM (`bootstrapMode`) reached the same one-kind conclusion, though their discriminators are per-NodeClass. Worth revisiting if the schemas diverge materially or mixed-mode deployments become a requirement.
