...
- **Cluster-wide default** — set `DEFAULT_NODEPOOL_SERVICE_ACCOUNT` on the controller (Helm
  `controller.env` or the `--default-nodepool-service-account` flag):

  ```sh
  # Example: Helm upgrade with the env var injected
